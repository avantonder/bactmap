...
workflow LONGREAD_MAPPING {
    ...
    CONSENSUS_BCFTOOLS ( BAM_SORT_STATS_SAMTOOLS.out.bam, VCF_FILTER.out.vcf, VCF_FILTER.out.index, ch_fasta )

    SEQTK_COMP( CONSENSUS_BCFTOOLS.out.consensus )

    emit:
    bam         = BAM_SORT_STATS_SAMTOOLS.out.bam   // channel: [ val(meta), [ bam ] ]
    bai         = BAM_SORT_STATS_SAMTOOLS.out.index // channel: [ val(meta), [ index ] ]
